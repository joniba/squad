...
16. **Teams notifications** — The coordinator MUST fire notifications via `.\scripts\notify-squad-event.ps1` for two categories of events. FAILURE: if notification fails, log it but don't block the workflow.
    ...
    - **Blocked on human/external** (IMMEDIATELY when detected — by coordinator or Ralph — that work is blocked on external dependencies, human decisions, or external system access): `.\scripts\notify-squad-event.ps1 -Event "blocked" -What "<what is blocked>" -Why "<why it needs Jonathan>" -ActionNeeded "<specific action>" -Link "<issue/PR URL>" -Agent "<agent who is blocked>"`. When multiple issues share a root cause, send ONE aggregated notification. Ralph's scan cycle MUST check for newly-detected blockers and fire notifications.
