...
> End-to-end example of adding a code reviewer to your squad and wiring their gate so it actually gets enforced. This walkthrough addresses a real failure: a reviewer was on our roster for weeks but never reviewed a single PR because the gate wasn't wired.
...
**What went wrong in our project:** Galadriel was on the roster as "Reviewer" from day one. Her charter said she reviews PRs. The routing table said "PR code review → Galadriel." But Multiple PRs were merged and issues were closed without her ever being spawned. Why?

Because the routing table says WHO handles what — it's for incoming requests ("review PR #42"). It does NOT say "after every agent completes work, route their output to Galadriel." The coordinator routes work TO agents, but nothing told it to route COMPLETED work to a reviewer. The "After Agent Work" flow in `squad.agent.md` says: collect results → present → spawn Scribe. No review step.

**The fix has three layers:**

| Layer | What it does | Where it lives |
|-------|-------------|----------------|
| Identity | Galadriel exists and knows how to review | `team.md` roster + `charter.md` |
| Routing | User can explicitly request "Galadriel, review this" | `routing.md` routing table |
| **Enforcement** | Coordinator MUST route every PR to Galadriel before merge | `routing.md` Rules section + `issue-lifecycle.md` post-work steps |
